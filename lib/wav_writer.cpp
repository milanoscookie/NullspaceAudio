...
bool WavWriter::open() {
  ...
  if (!file_.is_open()) {
    std::cerr << "WavWriter: failed to open file: " << path_ << '\n';
    return false;
  }
...
void WavWriter::writeHeader() {
  ...
  uint16_t audioFormat = (config_.bitsPerSample == 32) ? 3 : 1;
...
void WavWriter::writeBlock(const dsp::Block &block) {
...
void WavWriter::close() {
  ...
  std::cout << "WavWriter: saved " << path_ << '\n';
